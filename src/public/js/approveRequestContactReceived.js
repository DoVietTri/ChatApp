...
function approveRequestContactReceived() {
    $('.user-approve-contact-received').unbind('click').bind('click', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.success) {
                    ...
                    $(userInfo).find('div.user-approve-contact-received').remove();
                    ...
                    $(userInfo).find('div.contactPanel').append(`<div class="user-remove-contact action-danger" data-uid="${targetId}">
                                            Xóa liên hệ
                                        </div>`);
...
socket.on('response-approve-request-contact-received', function (user) {
    ...
    $('#contacts').find('ul')
        .prepend(`<li class="_contactList" data-uid="${user.id}">
